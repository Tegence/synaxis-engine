...
- portable Library skills with immutable instruction versions, generic bindings,
  append-only evaluations, private artifacts, and run provenance
...
The Engine-native Library stores reusable skills, artifacts, and provenance
without importing Platform concepts. A skill has stable metadata plus immutable
Markdown instruction versions and requested capabilities. Requested
capabilities are intent, not authority: an effective runtime set can only be
the intersection of the skill request, an optional binding ceiling, and the
already-granted runtime capabilities.
...
Artifacts have immutable text or Markdown versions (maximum 1 MiB). Every new
version is `pending` review. Runs retain only opaque actor/surface references,
...
instructions, evaluation annotations, artifact titles/summaries, and artifact
bodies. Structural IDs, digests, timestamps, and policy fields remain queryable
metadata. FileStore retains its documented plaintext local-development posture.
