...
- Set unique production values for `ENGINE_SECRET`, `ENGINE_PASSWORD`,
  `ENGINE_ENCRYPTION_KEY`, and any `SYNAXIS_ADMIN_TOKEN`.
...
- Hosted deployments should set `ENGINE_LOCAL_ADMIN_AUTH_ENABLED=false` and
  `SYNAXIS_ENABLE_LEGACY_ADMIN=false`; machine bearer authentication remains
