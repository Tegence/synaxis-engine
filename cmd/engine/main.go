...
func runEngine(rootCtx context.Context) error {
	...
	builtInCtx, builtInCancel := context.WithTimeout(runCtx, 30*time.Second)
	err = engine.ReconcileBuiltInLibrary(builtInCtx, store)
	builtInCancel()
	if err != nil {
		return fmt.Errorf("reconcile built-in Library: %w", err)
	}
	...
	libraryStore, _ := store.(engine.LibraryStore)
	registerBuiltinTools(s, auditSink, libraryStore)
	...
	gw.SetPublicURL(issuer) // builds the one-time decide_url on outbound webhook-v2 events (see notify.go)
	// Hosted-only (AND-09): tell Platform's device/email/Slack-interaction
	// fan-out about every structured event, over the same admin-token
	// credential Platform already uses to call INTO this Engine.
	gw.SetWorkspaceID(os.Getenv("SYNAXIS_WORKSPACE_ID"))
	gw.SetPlatformEvents(os.Getenv("ENGINE_EVENTS_URL"), adminToken)
	...
	// Host-attested Library results intentionally do not travel through the
	// browser/admin API or an MCP tool write. A configured external host signs
	// an exact JSON body with its per-client Ed25519 key; the Engine then
	// revalidates the live client epoch and selected immutable skill tuple in
	// one native store commit. This route has no CORS and is not BFF allowlisted.
	if runtimeStore, ok := store.(engine.LibraryRuntimeAttestationStore); ok {
		mux.Handle("/runtime/clients/{slug}/skill-runs", engine.NewLibraryRuntimeAttestationHandler(runtimeStore))
		// The signed activation fetch and run-correlation claim share the
		// same host key, signing scheme, and no-CORS posture. They are never
		// exposed on /api or the root /mcp surface.
		mux.Handle("/runtime/clients/{slug}/activation", engine.NewLibraryRuntimeActivationHandler(runtimeStore))
		if correlationStore, ok := store.(engine.LibraryRunCorrelationStore); ok {
			mux.Handle("/runtime/clients/{slug}/run-correlations", engine.NewLibraryRunCorrelationHandler(correlationStore))
		}
	}
	...
	provisionGeneration, err := provisionGenerationFromEnv()
	if err != nil {
		return fmt.Errorf("control contract configuration: %w", err)
	}
	consoleOptions := []engine.ConsoleOption{
		...
		// Workload clients get short-lived, resource-bound tokens brokered by
		// the Platform service actor; the AS re-checks the client binding
		// through the same authorizer RequireAuth uses before it signs.
		engine.WithWorkloadTokenIssuer(as.IssueResourceAccess),
		engine.WithUsageGate(usageGate),
		engine.WithEngineVersion(engineVersionFromEnv()),
		engine.WithProvisionGeneration(provisionGeneration),
	...
	if draftGenerator, err := skillDraftGeneratorFromEnv(security.hosted); err != nil {
		return fmt.Errorf("skill draft generator configuration: %w", err)
	} else if draftGenerator != nil {
		consoleOptions = append(consoleOptions, engine.WithSkillDraftGenerator(draftGenerator))
		log.Printf("engine: server-side skill draft generator enabled")
	}
	if actorVerifier != nil {
...
func registerBuiltinTools(s *server.MCPServer, sink engine.AuditSink, libraryStore engine.LibraryStore) {
	s.AddTool(
	...
	engine.RegisterLibraryArtifactTools(s, libraryStore, sink)
}
...
// engineVersionFromEnv is the build label reported on GET /control/v1/meta.
// An explicit ENGINE_VERSION wins; otherwise the module version or VCS
// revision embedded by the Go toolchain is used, falling back to "dev".
func engineVersionFromEnv() string {
...
// provisionGenerationFromEnv reports the hosted pinning generation for
// GET /control/v1/meta. Self-hosted Engines leave it unset and report 0; a
// set-but-malformed value is a configuration error rather than a silent 0.
func provisionGenerationFromEnv() (int64, error) {
	raw := strings.TrimSpace(os.Getenv("SYNAXIS_PROVISION_GENERATION"))
	if raw == "" {
		return 0, nil
	}
	generation, err := strconv.ParseInt(raw, 10, 64)
	if err != nil || generation <= 0 || strconv.FormatInt(generation, 10) != raw {
		return 0, errors.New("SYNAXIS_PROVISION_GENERATION must be a positive canonical integer")
	}
...
// skillDraftGeneratorFromEnv configures the optional self-hosted drafting
// seam. Hosted Engines must never receive a provider credential: their
// Platform owns the provider boundary and imports a typed editable candidate
// over the asserted control channel instead.
func skillDraftGeneratorFromEnv(hosted bool) (engine.SkillDraftGenerator, error) {
	endpoint := strings.TrimSpace(os.Getenv("ENGINE_SKILL_DRAFT_OPENAI_URL"))
	apiKey := strings.TrimSpace(os.Getenv("ENGINE_SKILL_DRAFT_OPENAI_API_KEY"))
	model := strings.TrimSpace(os.Getenv("ENGINE_SKILL_DRAFT_OPENAI_MODEL"))
	if hosted && (endpoint != "" || apiKey != "" || model != "") {
		return nil, errors.New("hosted Engine skill drafting must use the Platform broker, not ENGINE_SKILL_DRAFT_OPENAI_*")
	}
	...
	return engine.NewOpenAICompatibleSkillDraftGenerator(endpoint, apiKey, model)
}
