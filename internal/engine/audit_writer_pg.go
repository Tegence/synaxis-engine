...
func (w *auditWriter) enqueue(rec CallRecord) {
	...
	w.recordDrop("audit queue is full or shutting down", nil)
}
...
func (w *auditWriter) run() {
	...
	for rec := range w.queue {
		if w.ctx.Err() != nil {
			w.recordDrop("audit shutdown deadline expired", w.ctx.Err())
			w.discardQueued()
			return
		}
		w.persist(rec)
...
func (w *auditWriter) persist(rec CallRecord) {
	...
	for attempt := 1; attempt <= w.config.maxAttempts; attempt++ {
		if err := w.ctx.Err(); err != nil {
			w.recordDrop("audit shutdown cancelled a pending record", err)
			return
		}
		...
		if !w.waitForRetry(attempt) {
			w.recordDrop("audit shutdown cancelled a retry", w.ctx.Err())
			return
		}
	}
...
func (w *auditWriter) fail(rec CallRecord, err error, attempts int) {
	w.failures.Add(1)
	w.recordDrop(fmt.Sprintf("audit insert failed after %d attempt(s) for %s/%s", attempts, rec.Account, rec.Tool), err)
}

func (w *auditWriter) recordDrop(reason string, err error) {
	n := w.dropped.Add(1)
	if err != nil {
