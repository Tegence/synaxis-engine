...
var encBytesPrefix = []byte("encb:v1:")
...
// EncryptBytes is the bytea counterpart to Encrypt. It uses the same
// AES-256-GCM key and fail-closed semantics, but keeps binary data binary so
// callers such as Library image blobs do not need an extra base64 layer in
// PostgreSQL. A nil cipher retains the documented plaintext development mode.
func (c *Cipher) EncryptBytes(plain []byte) ([]byte, error) {
	if len(plain) == 0 {
		return nil, nil
	}
	if c == nil {
		if bytes.HasPrefix(plain, encBytesPrefix) {
			return nil, fmt.Errorf("%w: ENGINE_ENCRYPTION_KEY is required to preserve encrypted data", ErrCipherUnavailable)
		}
	...
	if bytes.HasPrefix(plain, encBytesPrefix) {
		if _, err := c.DecryptBytes(plain); err != nil {
			return nil, err
		}
		return append([]byte(nil), plain...), nil
	}
	...
	ciphertext := c.aead.Seal(nonce, nonce, plain, nil)
	out := make([]byte, 0, len(encBytesPrefix)+len(ciphertext))
	out = append(out, encBytesPrefix...)
	out = append(out, ciphertext...)
	return out, nil
}

// DecryptBytes reverses EncryptBytes. Legacy unprefixed bytea data is treated
// as plaintext only in the same way Decrypt handles pre-encryption text rows.
func (c *Cipher) DecryptBytes(value []byte) ([]byte, error) {
	if !bytes.HasPrefix(value, encBytesPrefix) {
		return append([]byte(nil), value...), nil
	}
	if c == nil || c.aead == nil {
		return nil, fmt.Errorf("%w: ENGINE_ENCRYPTION_KEY is required to read encrypted data", ErrCipherUnavailable)
	}
	raw := value[len(encBytesPrefix):]
