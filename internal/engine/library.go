...
type LibraryStore interface {
	...
	// LibrarySkillRuns returns the newest skill_run provenance for one skill,
	// across every version, bounded by limit. It is evidence for the Console:
	// no bodies, traces, or capability grants are derived from it.
	LibrarySkillRuns(context.Context, string, int) ([]LibraryRun, error)

	// Artifact drafts mirror skill drafts: an editable generated candidate
	// with digest-only prompt provenance and an optional exact source
	// citation. A draft is never an artifact, run, grant, or publication.
	CreateLibraryArtifactDraft(context.Context, LibraryArtifactDraft) (LibraryArtifactDraft, error)
	LibraryArtifactDraft(context.Context, string) (LibraryArtifactDraft, bool)
	ImportPlatformLibraryArtifactDraft(context.Context, LibraryPlatformArtifactDraftImport, string) (draft LibraryArtifactDraft, replayed bool, err error)
	...
	// CreateLibraryHumanArtifactWithInitialVersion is the Console write for a
	// human-origin artifact that cites one exact source version. It derives
	// the human run from the authenticated actor and commits run, artifact,
	// and first version together, so a citation never dangles without the run
	// that recorded it.
	CreateLibraryHumanArtifactWithInitialVersion(context.Context, LibraryArtifact, LibraryArtifactVersion) (LibraryRun, LibraryArtifact, LibraryArtifactVersion, error)
	// LibraryArtifactCitations lists the newest artifacts whose immutable
	// source citation names this artifact, bounded by limit. It is a Console
	// detail projection and never returns bodies.
	LibraryArtifactCitations(context.Context, string, int) ([]LibraryArtifactCitation, error)
	...
	// ReviewLibraryArtifactVersionWithComment is the review path with an
	// optional bounded reviewer note. The note is private authored text: it
	// is encrypted at rest and never part of a publication candidate.
	ReviewLibraryArtifactVersionWithComment(context.Context, string, string, string, string, string, time.Time) (LibraryArtifactVersion, error)
...
type LibraryArtifactMediaStore interface {
	...
	// The human image methods back the administrator Console. They derive a
	// human run (or append to an existing image artifact) from the
	// authenticated actor and commit version plus blob in one save/transaction.
	CreateLibraryHumanImageArtifactWithInitialVersion(context.Context, LibraryArtifact, libraryArtifactImageCanonical, string) (LibraryRun, LibraryArtifact, LibraryArtifactVersion, error)
	CreateLibraryHumanImageArtifactVersion(context.Context, LibraryHumanImageArtifactVersionCreateRequest) (LibraryArtifactVersion, error)
...
const (
	...
	// PostgreSQL stores binding priority in an INT column. Keep authored values
	// portable across both stores while reserving the top INT32 value for the
	// Engine-owned operating guide.
	libraryMinAuthoredBindingPriority = -1 << 31
	libraryMaxAuthoredBindingPriority = (1 << 31) - 2
	...
	// Version provenance records how an immutable version came to exist.
	// "generated" is server-derived from a generated draft; a caller may only
	// claim "manual" or "imported".
	LibraryVersionOriginGenerated = "generated"
	LibraryVersionOriginManual    = "manual"
	LibraryVersionOriginImported  = "imported"

	// libraryHumanConsoleSurfaceRef names the administrator Console as the
	// surface of a derived human run. It is a label, not an agent surface ID.
	libraryHumanConsoleSurfaceRef = "console"
...
const (
	...
	// Draft rationale/assumptions, version changelogs, and review comments are
	// bounded authored text. Oversize values are rejected, never truncated,
	// so a stored record always equals what its author or generator produced.
	libraryMaxRationaleBytes                  = 2 << 10
	libraryMaxAssumptionBytes                 = 240
	libraryMaxAssumptions                     = 8
	libraryMaxChangelogBytes                  = 1 << 10
	libraryMaxReviewCommentBytes              = 2 << 10
	libraryArtifactDraftSourceContextMaxBytes = 256 << 10
	librarySkillEvidenceRunLimit              = 20
	libraryArtifactCitedByLimit               = 20
...
var (
	...
	ErrLibraryBindingGenerationExhausted = errors.New("library skill binding generation is exhausted")
	...
	ErrLibraryArtifactDraftNotFound      = errors.New("library artifact draft not found")
	// ErrLibraryArtifactDraftGeneratorUnavailable is distinct from the skill
	// generator error: a configured generator may support skills only.
	ErrLibraryArtifactDraftGeneratorUnavailable = errors.New("artifact draft generator is unavailable")
	// ErrLibraryArtifactFormatMismatch keeps an artifact's version history in
	// one media class. Text and image versions never interleave.
	ErrLibraryArtifactFormatMismatch = errors.New("library artifact versions cannot switch between text and image")
	// ErrLibraryArtifactSourceDigestMismatch means the cited version exists
	// but its immutable digest is not the one the caller observed.
	ErrLibraryArtifactSourceDigestMismatch = errors.New("library artifact source digest does not match the stored version")
)
...
type LibrarySkillVersion struct {
	...
	// Changelog is an optional bounded author note for this version.
	Changelog string `json:"changelog,omitempty"`
	// Provenance is absent for versions written before it was recorded.
	Provenance *LibraryVersionProvenance `json:"provenance,omitempty"`
	// Files is the sorted bundle manifest. SKILL.md is always present and
	// mirrors Content/Digest; other entries reference content-addressed blobs.
	// A version written before bundles existed is materialized as the
	// one-file manifest, so Files is never empty on a stored version.
	Files []LibrarySkillFile `json:"files"`
	// ManifestDigest is deterministic over Files (path, media type, size, and
	// file digest); it never covers IDs, timestamps, or authorship.
	ManifestDigest string    `json:"manifestDigest"`
	CreatedBy      string    `json:"createdBy,omitempty"`
	CreatedAt      time.Time `json:"createdAt"`
}

// LibraryVersionProvenance says how one immutable skill or artifact version
// came to exist. A generated origin carries the draft's generator/model labels
// and prompt digest; the raw prompt is never part of provenance.
type LibraryVersionProvenance struct {
...
// Rationale and assumptions are the generator's own account of what it did
// where the brief was silent; they are review aids, not instructions.
...
// LibraryArtifactDraft is the output counterpart of a skill draft: an
// editable generated candidate body that is not yet an artifact, run, grant,
// or publication. The optional source tuple is the exact immutable version the
// generator was shown as context.
type LibraryArtifactDraft struct {
	ID                      string    `json:"id"`
...
// LibraryConsoleArtifactVersionSummary is the latest-version metadata a
// management list needs. It never carries a body or reviewer identity; the
// MIME type is present only for image versions and comes from blob metadata.
type LibraryConsoleArtifactVersionSummary struct {
	ID              string
	Version         int
	Format          string
	MIMEType        string
...
type LibraryConsoleArtifact struct {
	Artifact      LibraryArtifact
	LatestVersion *LibraryConsoleArtifactVersionSummary
}

// LibraryConsoleArtifactPage is the bounded management list. It carries the
// latest immutable version's metadata but never a version body.
type LibraryConsoleArtifactPage struct {
	Artifacts  []LibraryConsoleArtifact
	NextCursor LibraryConsolePageCursor
}

// LibraryArtifactCitation names one artifact that cites another as its exact
// immutable source. VersionID is the cited (source) version.
type LibraryArtifactCitation struct {
...
type LibraryArtifactVersion struct {
	...
	// Changelog and Provenance mirror the skill version fields; both are
	// absent on versions written before they were recorded.
	Changelog  string                    `json:"changelog,omitempty"`
	Provenance *LibraryVersionProvenance `json:"provenance,omitempty"`
	...
	// ReviewComment is the reviewer's bounded private note. It is encrypted at
	// rest and stripped from every MCP/metadata projection.
...
type LibraryArtifactGrant struct {
	...
	// A live grant has no revocation timestamp. omitempty does not omit a
	// zero-valued time.Time, which would otherwise look revoked to API clients.
...
// ArtifactDraftGenerator is the output counterpart of SkillDraftGenerator.
// A generator may implement both; the Console discovers artifact support by
// type assertion so an older skill-only generator keeps working unchanged.
type ArtifactDraftGenerator interface {
	GenerateArtifactDraft(context.Context, ArtifactDraftGenerationRequest) (ArtifactDraftGeneration, error)
}

// ArtifactDraftGenerationRequest carries the bounded brief plus, when the
// caller cited a source, that source's title and body as read-only context.
// The Console bounds the source body before it reaches a generator.
type ArtifactDraftGenerationRequest struct {
	Title       string
	Summary     string
	Prompt      string
	Format      string
...
type ArtifactDraftGeneration struct {
	Title       string
	Summary     string
	Content     string
	Format      string
...
// LibraryHumanImageArtifactVersionCreateRequest appends one canonical image
// version to an existing image artifact on behalf of the authenticated
// administrator. The store re-reads the head under its own lock and refuses
// to append an image to a text artifact.
type LibraryHumanImageArtifactVersionCreateRequest struct {
	ArtifactID string
	Image      libraryArtifactImageCanonical
	AltText    string
	CreatedBy  string
	Changelog  string
	Provenance *LibraryVersionProvenance
}
...
func newLibraryArtifactDraftID() string   { return "libad_" + newEpoch() }
...
func validateLibrarySkillVersion(version LibrarySkillVersion) error {
	...
	if err := validateLibraryChangelog(version.Changelog); err != nil {
		return err
	}
	if err := validateLibraryVersionProvenance(version.Provenance); err != nil {
		return err
	}
	return validateLibraryOpaqueRef("created by", version.CreatedBy, true)
}

// validateLibraryVersionProvenance accepts a nil provenance (legacy or
// unrecorded) and otherwise requires a known origin. A generated origin must
// name its generator, exactly like a generated draft.
func validateLibraryVersionProvenance(provenance *LibraryVersionProvenance) error {
	if provenance == nil {
		return nil
	}
	switch provenance.Origin {
	case LibraryVersionOriginGenerated:
		if strings.TrimSpace(provenance.Generator) == "" {
			return errors.New("generated version provenance requires a generator")
		}
	case LibraryVersionOriginManual, LibraryVersionOriginImported:
	default:
		return fmt.Errorf("invalid version origin %q", provenance.Origin)
	}
	if len(provenance.Generator) > libraryMaxNameBytes || len(provenance.Model) > libraryMaxNameBytes {
		return errors.New("version provenance generator metadata is too long")
	}
	if err := validateLibraryDigest("prompt", provenance.PromptDigest, true); err != nil {
		return err
	}
	return validateLibraryOpaqueRef("draft", provenance.DraftID, true)
}

func validateLibraryChangelog(changelog string) error {
	if len(changelog) > libraryMaxChangelogBytes || !utf8.ValidString(changelog) {
		return fmt.Errorf("changelog must be valid UTF-8 of at most %d bytes", libraryMaxChangelogBytes)
	}
	return nil
}

func validateLibraryReviewComment(comment string) error {
	if len(comment) > libraryMaxReviewCommentBytes || !utf8.ValidString(comment) {
		return fmt.Errorf("review comment must be valid UTF-8 of at most %d bytes", libraryMaxReviewCommentBytes)
	}
	return nil
}

func validateLibraryRationale(rationale string) error {
	if len(rationale) > libraryMaxRationaleBytes || !utf8.ValidString(rationale) {
		return fmt.Errorf("rationale must be valid UTF-8 of at most %d bytes", libraryMaxRationaleBytes)
	}
	return nil
}

// normalizeLibraryDraftAssumptions trims and drops blank entries but rejects,
// rather than truncates, an oversize list or entry: a stored assumption must
// be exactly what the generator or importer said.
func normalizeLibraryDraftAssumptions(values []string) ([]string, error) {
	...
	for _, raw := range values {
		...
		if len(value) > libraryMaxAssumptionBytes || !utf8.ValidString(value) {
			return nil, fmt.Errorf("each assumption must be valid UTF-8 of at most %d bytes", libraryMaxAssumptionBytes)
		}
		out = append(out, value)
	}
	if len(out) > libraryMaxAssumptions {
		return nil, fmt.Errorf("at most %d assumptions are allowed", libraryMaxAssumptions)
	}
	return out, nil
}
...
func validateLibrarySkillDraft(draft LibrarySkillDraft) error {
	...
	if err := validateLibraryRationale(draft.Rationale); err != nil {
		return err
	}
	if _, err := normalizeLibraryDraftAssumptions(draft.Assumptions); err != nil {
		return err
	}
	return validateLibraryOpaqueRef("created by", draft.CreatedBy, true)
}

// validateLibraryArtifactDraft mirrors the skill draft rules for an output
// candidate. The source tuple is all-or-nothing; the store proves it names a
// stored version and digest before persisting the draft.
func validateLibraryArtifactDraft(draft LibraryArtifactDraft) error {
	if strings.TrimSpace(draft.Title) == "" || len(draft.Title) > libraryMaxNameBytes {
		return fmt.Errorf("draft title is required and must be at most %d bytes", libraryMaxNameBytes)
	}
	if len(draft.Summary) > libraryMaxDescriptionBytes || len(draft.Content) > libraryMaxContentBytes || strings.TrimSpace(draft.Content) == "" {
		return errors.New("draft content or summary is invalid")
	}
	if draft.Format != LibraryArtifactFormatMarkdown && draft.Format != LibraryArtifactFormatText {
		return fmt.Errorf("invalid draft format %q", draft.Format)
	}
	if draft.Origin != LibraryDraftOriginGenerated && draft.Origin != LibraryDraftOriginManual {
		return fmt.Errorf("invalid draft origin %q", draft.Origin)
	}
	...
	if err := validateLibraryArtifactSourceReference(draft.SourceArtifactID, draft.SourceArtifactVersionID, draft.SourceArtifactDigest, true); err != nil {
		return err
	}
	return validateLibraryOpaqueRef("created by", draft.CreatedBy, true)
}

func normalizedLibraryArtifactDraft(draft LibraryArtifactDraft) (LibraryArtifactDraft, error) {
...
// libraryVersionProvenanceFromSkillDraft derives the immutable provenance a
// version inherits when it is created from a stored draft. A manual draft
// yields a manual origin; the caller never chooses "generated" itself.
func libraryVersionProvenanceFromSkillDraft(draft LibrarySkillDraft) *LibraryVersionProvenance {
	origin := LibraryVersionOriginManual
	if draft.Origin == LibraryDraftOriginGenerated {
		origin = LibraryVersionOriginGenerated
	}
	return &LibraryVersionProvenance{Origin: origin, Generator: draft.Generator, Model: draft.Model, PromptDigest: draft.PromptDigest, DraftID: draft.ID}
}

func libraryVersionProvenanceFromArtifactDraft(draft LibraryArtifactDraft) *LibraryVersionProvenance {
	origin := LibraryVersionOriginManual
	if draft.Origin == LibraryDraftOriginGenerated {
		origin = LibraryVersionOriginGenerated
	}
...
// libraryVersionProvenanceFromOrigin accepts only the origins a caller may
// claim without a draft. "generated" is reserved for the draft path.
func libraryVersionProvenanceFromOrigin(origin string) (*LibraryVersionProvenance, error) {
	switch origin {
	case "", LibraryVersionOriginManual:
		return &LibraryVersionProvenance{Origin: LibraryVersionOriginManual}, nil
	case LibraryVersionOriginImported:
		return &LibraryVersionProvenance{Origin: LibraryVersionOriginImported}, nil
	case LibraryVersionOriginGenerated:
		return nil, errors.New("a generated origin requires a draftId")
	default:
		return nil, fmt.Errorf("invalid version origin %q", origin)
	}
}

func copyLibraryVersionProvenance(provenance *LibraryVersionProvenance) *LibraryVersionProvenance {
	if provenance == nil {
		return nil
	}
	copy := *provenance
...
func validateLibraryBinding(binding LibrarySkillBinding) error {
	...
	if binding.Priority < libraryMinAuthoredBindingPriority || binding.Priority > libraryMaxAuthoredBindingPriority {
		return fmt.Errorf("binding priority must be between %d and %d", libraryMinAuthoredBindingPriority, libraryMaxAuthoredBindingPriority)
	}
...
func validateLibraryRun(run LibraryRun) error {
	...
	case LibraryRunOriginAgentDirect, LibraryRunOriginAutomation, LibraryRunOriginHuman:
		...
		if err := validateLibraryArtifactSourceReference(run.SourceArtifactID, run.SourceArtifactVersionID, run.SourceArtifactDigest, libraryOriginMayCiteSource(run.Origin)); err != nil {
			return err
		}
	default:
...
func validateLibraryArtifact(artifact LibraryArtifact) error {
	...
	case LibraryArtifactOriginAgentDirect, LibraryArtifactOriginAutomation, LibraryArtifactOriginHuman:
		...
		if err := validateLibraryArtifactSourceReference(artifact.SourceArtifactID, artifact.SourceArtifactVersionID, artifact.SourceArtifactDigest, libraryOriginMayCiteSource(artifact.Origin)); err != nil {
			return err
		}
...
// libraryOriginMayCiteSource limits exact source citations to the two
// provenance classes whose run records the citation: a direct agent action
// and a human Console action. Automation and skill runs remain uncited.
func libraryOriginMayCiteSource(origin string) bool {
	return origin == LibraryArtifactOriginAgentDirect || origin == LibraryArtifactOriginHuman
}
...
func validateLibraryArtifactSourceReference(artifactID, versionID, digest string, allow bool) error {
	...
	if !allow {
		return errors.New("artifact source references are allowed only for direct agent or human provenance")
	}
...
// normalizeLibraryDigestInput accepts the two spellings callers use for a
// SHA-256 value, bare hex or "sha256:"-prefixed, and returns the bare
// lowercase form every stored digest uses. It does not validate.
func normalizeLibraryDigestInput(value string) string {
...
func normalizedLibraryVersion(version LibrarySkillVersion) (LibrarySkillVersion, error) {
	...
	// A caller that supplies no manifest is writing the legacy single-document
	// shape; a bundle caller supplies the manifest it normalized. Either way
	// the stored version carries a validated manifest and its digest.
	version, err = librarySkillVersionWithManifest(version)
	if err != nil {
		return LibrarySkillVersion{}, err
	}
...
func normalizedLibraryArtifactVersion(version LibraryArtifactVersion) (LibraryArtifactVersion, error) {
	...
	version.Provenance = copyLibraryVersionProvenance(version.Provenance)
	if version.CreatedAt.IsZero() {
		version.CreatedAt = time.Now().UTC()
	}
	return version, nil
...
// prepareLibraryHumanArtifact derives the human run that records a Console
// artifact creation with an exact source citation. The caller supplies the
// authenticated actor as CreatedBy; it cannot nominate a run ID, surface, or
// agent-surface projection, and the run carries no effective capabilities.
func prepareLibraryHumanArtifact(artifact LibraryArtifact, version LibraryArtifactVersion) (LibraryRun, LibraryArtifact, LibraryArtifactVersion, error) {
	if artifact.Origin != LibraryArtifactOriginHuman || artifact.ID != "" || artifact.RunID != "" || artifact.AgentSurfaceID != "" {
		return LibraryRun{}, LibraryArtifact{}, LibraryArtifactVersion{}, errors.New("human artifact must derive its run provenance")
	}
	...
	if version.ID != "" || version.ArtifactID != "" || version.Version != 0 {
		return LibraryRun{}, LibraryArtifact{}, LibraryArtifactVersion{}, errors.New("human artifact version must derive immutable identity")
	}
...
func copyLibrarySkillVersion(version LibrarySkillVersion) LibrarySkillVersion {
	...
	version.Provenance = copyLibraryVersionProvenance(version.Provenance)
	version.Files = copyLibrarySkillFiles(version.Files)
	return version
}
...
func copyLibrarySkillDraft(draft LibrarySkillDraft) LibrarySkillDraft {
	...
	draft.Assumptions = append([]string{}, draft.Assumptions...)
	return draft
}

func copyLibraryArtifactDraft(draft LibraryArtifactDraft) LibraryArtifactDraft {
	draft.Assumptions = append([]string{}, draft.Assumptions...)
	return draft
}
