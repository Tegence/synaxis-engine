...
func TestBuildLibrarySkillActivationBundleEnforcesPortableRuntimeBounds(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			selections: activationLimitSelections(agentSurfaceID, 33, "# bounded"),
		},
		{
		...
		{
			// Four 32 KiB bodies fit the raw byte counter exactly, but their
			// metadata must still fit the portable 128 KiB canonical context.
			name:       "canonical context bytes",
			selections: activationLimitSelections(agentSurfaceID, 4, maximumInstruction),
		},
	}
