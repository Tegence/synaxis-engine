...
WHERE version.id=$1`, updated.Version.ID).Scan(&rawBody, &rawCreatedBy, &rawAltText, &rawData); err != nil {
	...
	if rawBody == "" || !strings.HasPrefix(rawBody, encPrefix) || !strings.HasPrefix(rawCreatedBy, encPrefix) || !strings.HasPrefix(rawAltText, encPrefix) || !strings.HasPrefix(string(rawData), encPrefix) || rawCreatedBy == updated.Version.CreatedBy {
