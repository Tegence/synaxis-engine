...
// write: inline files with bytes, staged references the store resolves,
// whether the caller authored a bundle (which makes SKILL.md front matter
// mandatory), and whether it sent a file list at all. An explicit empty list
// is declared but not authored: it deliberately removes every bundle file.
...
func normalizeLibrarySkillAuthoringFiles(content string, inputs []LibrarySkillFileInput) (string, librarySkillAuthoringBundleInput, error) {
	...
	if len(inputs) == 0 {
		return content, librarySkillAuthoringBundleInput{declared: true}, nil
	}
	...
	return content, librarySkillAuthoringBundleInput{inline: kept, refs: refs, authored: true, declared: true}, nil
}
...
func normalizeLibraryMCPClientSkillAuthoringUpdateRequest(request LibraryMCPClientSkillAuthoringUpdateRequest) (LibraryMCPClientSkillAuthoringUpdateRequest, string, error) {
	...
	canonical, err := json.Marshal(struct {
		...
		RequestedCapabilities []string `json:"requestedCapabilities"`
		Files                 []string `json:"files,omitempty"`
		// An empty declared list removes every bundle file while an omitted
		// list is refused against a head that has files, so the two must not
		// replay as each other. The marker appears only in that case, which
		// keeps every other update's payload digest unchanged.
		EmptyFilesDeclared bool `json:"emptyFilesDeclared,omitempty"`
	}{
		...
		Files:                 files,
		EmptyFilesDeclared:    bundle.declared && len(files) == 0,
	})
	if err != nil {
