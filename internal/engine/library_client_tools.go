...
func registerMCPClientLibraryTools(s *server.MCPServer, store LibraryStore, sink AuditSink, client MCPClient, live func(context.Context) bool) []string {
	...
	s.AddTool(
		mcp.NewTool(
			"library_skill_update",
			mcp.WithDescription("Append one immutable version to a Library skill this exact MCP client created, or to the one exact skill an owner/admin temporarily delegated to it. The owner/admin-issued authoring window must be active; the client binding and selected head must still match the lease. Supply the exact current version ID and digest from library_skill_authoring_list or a prior result. Nothing carries forward from the current version: when it has bundle files, send the complete files list to keep them or an empty list to remove them; an update that omits files is then refused without consuming a write. This consumes one bounded authoring write and cannot change names, bindings, publication, credentials, OAuth scopes, tools, or authority."),
...
type libraryMCPClientSkillUpdateInput struct {
	...
	RequestedCapabilities []string                `json:"requestedCapabilities,omitempty" jsonschema:"Requested capability intent only; this never grants authority"`
	Files                 []LibrarySkillFileInput `json:"files,omitempty" jsonschema:"Bundle files, as for library_skill_create; the whole manifest of the new version is exactly SKILL.md plus these entries, and nothing carries forward from the current version. Required when the current version has bundle files: send the complete list to keep them (unchanged files may be re-sent or re-staged), or an empty list to remove them all. Omitting it against such a version is rejected without consuming a write."`
}
...
func libraryMCPClientSkillAuthoringErrorResult(err error, fallback string) *mcp.CallToolResult {
	...
	case errors.Is(err, ErrLibrarySkillBundleInvalid),
		...
		// Raised only after the caller matched the exact current version ID and
		// digest, so naming the head's file count reveals nothing new.
		errors.Is(err, ErrLibrarySkillFilesOmitted):
