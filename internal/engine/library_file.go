...
func (s *FileStore) CreateLibrarySkillWithInitialVersion(_ context.Context, skill LibrarySkill, version LibrarySkillVersion) (LibrarySkill, LibrarySkillVersion, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.createLibrarySkillWithInitialVersionLocked(skill, version, nil)
}

// createLibrarySkillWithInitialVersionLocked commits a skill, its first
// immutable version, and (for a bundle) the blobs that version references in
// one save. A nil bundle is the legacy single-document write; its manifest is
// the derived one-file manifest.
func (s *FileStore) createLibrarySkillWithInitialVersionLocked(skill LibrarySkill, version LibrarySkillVersion, bundle *LibrarySkillBundle) (LibrarySkill, LibrarySkillVersion, error) {
	...
	if bundle != nil {
		if err := s.requireLibrarySkillBlobsLocked(*bundle); err != nil {
			return LibrarySkill{}, LibrarySkillVersion{}, err
		}
...
func (s *FileStore) CreateLibrarySkillVersion(_ context.Context, version LibrarySkillVersion) (LibrarySkillVersion, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.createLibrarySkillVersionLocked(version, nil)
}

func (s *FileStore) createLibrarySkillVersionLocked(version LibrarySkillVersion, bundle *LibrarySkillBundle) (LibrarySkillVersion, error) {
	...
	if err := requireLibrarySkillFilesDeclared(s.latestLibrarySkillVersionLocked(skill.ID), libraryBundleDeclaresFiles(bundle)); err != nil {
	...
	var addedBlobs []string
	if bundle != nil {
		addedBlobs = s.storeLibrarySkillBlobsLocked(bundle.Blobs, normalized.CreatedAt)
	}
	copy := copyLibrarySkillVersion(normalized)
	previousUpdatedAt, previousDescription := skill.UpdatedAt, skill.Description
	skill.UpdatedAt = normalized.CreatedAt
	if bundle != nil && bundle.SkillDescription != nil {
		skill.Description = *bundle.SkillDescription
	}
	s.librarySkillVersions = append(s.librarySkillVersions, &copy)
	if err := s.saveLocked(); err != nil {
		s.librarySkillVersions = s.librarySkillVersions[:len(s.librarySkillVersions)-1]
		skill.UpdatedAt, skill.Description = previousUpdatedAt, previousDescription
		s.removeLibrarySkillBlobsLocked(addedBlobs)
		return LibrarySkillVersion{}, err
	}
