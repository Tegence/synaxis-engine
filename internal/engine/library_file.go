...
func (s *FileStore) UpsertLibrarySkillBinding(_ context.Context, binding LibrarySkillBinding) (LibrarySkillBinding, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.librarySkillLocked(binding.SkillID) == nil {
...
func (s *FileStore) LibraryConsoleArtifactPage(_ context.Context, cursor LibraryConsolePageCursor, limit int) (LibraryConsoleArtifactPage, error) {
	...
	items := make([]LibraryArtifact, 0, len(s.libraryArtifacts))
	for _, artifact := range s.libraryArtifacts {
		if artifact == nil {
			continue
		}
		...
		items = append(items, copyLibraryArtifact(*artifact))
	}
	sort.Slice(items, func(i, j int) bool {
		if items[i].CreatedAt.Equal(items[j].CreatedAt) {
			return items[i].ID > items[j].ID
	...
	page := LibraryConsoleArtifactPage{Artifacts: make([]LibraryArtifact, 0, min(limit, len(items)))}
	if len(items) > limit {
		last := items[limit-1]
...
func (s *FileStore) ReviewLibraryArtifactVersion(_ context.Context, artifactID, versionID, status, reviewedBy string, reviewedAt time.Time) (LibraryArtifactVersion, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.libraryArtifactLocked(artifactID) == nil {
		return LibraryArtifactVersion{}, ErrLibraryArtifactNotFound
	...
	version.RedactionStatus, version.ReviewedBy = status, reviewedBy
