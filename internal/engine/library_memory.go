...
func normalizeLibraryMemoryVersion(version LibraryMemoryVersion) (LibraryMemoryVersion, error) {
	...
	// A version's creation time also timestamps its memory head and automatic
	// grant revocations. Use PostgreSQL's precision in both stores so returned
	// and reloaded records identify the exact same correction instant.
	version.CreatedAt = version.CreatedAt.Truncate(time.Microsecond)
	return version, nil
}
