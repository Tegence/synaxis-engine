...
// A correction uses one persistable instant for its immutable version and head.
func assertLibraryMemoryVersionTimes(t *testing.T, store LibraryMemoryStore, memory LibraryMemory, version LibraryMemoryVersion, wantCreatedAt time.Time) {
	t.Helper()
	if !version.CreatedAt.Equal(wantCreatedAt) {
		t.Fatalf("version creation time=%s want=%s", version.CreatedAt.Format(time.RFC3339Nano), wantCreatedAt.Format(time.RFC3339Nano))
	}
	loadedVersion, found := store.LibraryMemoryVersion(context.Background(), memory.ID, version.ID)
	if !found || !loadedVersion.CreatedAt.Equal(version.CreatedAt) {
		t.Fatalf("version timestamp changed after persistence: returned=%s loaded=%s found=%t", version.CreatedAt.Format(time.RFC3339Nano), loadedVersion.CreatedAt.Format(time.RFC3339Nano), found)
	}
	loadedMemory, found := store.LibraryMemory(context.Background(), memory.ID)
	if !found || !loadedMemory.UpdatedAt.Equal(version.CreatedAt) || !memory.UpdatedAt.Equal(version.CreatedAt) {
		t.Fatalf("head timestamp differs from its version: returned=%s loaded=%s version=%s found=%t", memory.UpdatedAt.Format(time.RFC3339Nano), loadedMemory.UpdatedAt.Format(time.RFC3339Nano), version.CreatedAt.Format(time.RFC3339Nano), found)
	}
}
...
func TestFileLibraryMemoryLifecycleIsolationGrantExpiryCorrectionAndForget(t *testing.T) {
	...
	}, LibraryMemoryVersion{Content: "Use the versioned gateway contract.", CreatedAt: time.Date(2026, 8, 31, 10, 0, 0, 123456789, time.UTC)})
	if err != nil {
		t.Fatal(err)
	}
	...
	corrected, second, err := store.CreateLibraryMemoryVersion(ctx, memory.ID, LibraryMemoryVersion{Content: "Use control contract v2.", CreatedAt: correctionTime}, "owner")
	if err != nil {
		t.Fatal(err)
	}
	...
	assertLibraryMemoryVersionTimes(t, reopened, corrected, second, correctionTime.Add(-789*time.Nanosecond))
	grants, err := reopened.LibraryMemoryGrants(ctx, memory.ID)
...
func TestFileLibraryConsoleMemoryPageStableKeyset(t *testing.T) {
	...
	// Use the same ordering edge cases as the PostgreSQL parity regression.
