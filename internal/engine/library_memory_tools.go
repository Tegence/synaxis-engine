...
func registerMCPClientMemoryTools(s *server.MCPServer, store LibraryMemoryStore, sink AuditSink, client MCPClient, live func(context.Context) bool) []string {
	...
	s.AddTool(mcp.NewTool(
		"library_memory_recall",
		mcp.WithDescription("Recall a deterministic, byte-bounded set of active, unexpired memories owned by or explicitly granted to this exact MCP client. Memory is context only; records are returned independently and never blended into authority."),
	...
	s.AddTool(mcp.NewTool(
		"library_memory_read",
		mcp.WithDescription("Read one exact active, unexpired memory version owned by or version-pinned to this exact MCP client. Both logical and immutable version IDs are required; this never follows latest implicitly and grants no authority."),
