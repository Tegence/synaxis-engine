...
func TestPgLibraryConsolePages(t *testing.T) {
	...
	if err != nil || len(artifactPage.Artifacts) != 1 || artifactPage.Artifacts[0].Artifact.ID != artifactIDs[2] || artifactPage.NextCursor.ID == "" {
		t.Fatalf("first Postgres Console artifact page=%+v err=%v", artifactPage, err)
	}
	if latest := artifactPage.Artifacts[0].LatestVersion; latest == nil || latest.Version != 1 || latest.Format == "" || latest.MIMEType != "" || latest.RedactionStatus != LibraryRedactionPending {
		t.Fatalf("first Postgres Console artifact page latest summary=%+v", latest)
	}
	artifactPage, err = store.LibraryConsoleArtifactPage(ctx, artifactPage.NextCursor, 1)
	if err != nil || len(artifactPage.Artifacts) != 1 || artifactPage.Artifacts[0].Artifact.ID != artifactIDs[1] || artifactPage.NextCursor.ID == "" {
		t.Fatalf("second Postgres Console artifact page=%+v err=%v", artifactPage, err)
	}
	artifactPage, err = store.LibraryConsoleArtifactPage(ctx, artifactPage.NextCursor, 1)
	if err != nil || len(artifactPage.Artifacts) != 1 || artifactPage.Artifacts[0].Artifact.ID != artifactIDs[0] || artifactPage.NextCursor.ID != "" {
