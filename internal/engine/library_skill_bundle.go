...
var (
	...
	// ErrLibrarySkillFilesOmitted refuses a version append that sends no file
	// list while the current version has bundle files. Each version's manifest
	// is exactly what its write sent, so such a write would silently publish a
	// head without those files; see requireLibrarySkillFilesDeclared.
	ErrLibrarySkillFilesOmitted = errors.New("library skill version omits the current version's bundle files")
)
...
type LibrarySkillBundle struct {
	...
	// FilesDeclared records that the caller sent a file list, even an empty
	// one, so the bundle is the complete intended manifest. A bundle that
	// carries any file besides SKILL.md is declared by construction.
	FilesDeclared bool
}
...
// librarySkillBundleFileCount counts the manifest entries besides SKILL.md.
func librarySkillBundleFileCount(files []LibrarySkillFile) int {
	count := 0
	for _, file := range files {
		if file.Path != LibrarySkillInstructionsPath {
			count++
...
// requireLibrarySkillFilesDeclared is the guard every version append runs
// against the current head, under the same lock or transaction that appends.
// A version's manifest is exactly what its write sent and is never carried
// forward, so a write that sends no file list at all is ambiguous against a
// head that has bundle files: accepting it would publish a SKILL.md-only head
// that tracking bindings resolve at once. Such a write is refused. Sending the
// complete list keeps the files; an explicit empty list removes them. Heads
// without bundle files (every pre-bundle skill) accept text-only writes as
// before.
func requireLibrarySkillFilesDeclared(current *LibrarySkillVersion, declared bool) error {
	if declared || current == nil {
		return nil
	}
	count := librarySkillBundleFileCount(current.Files)
	if count == 0 {
		return nil
	}
	noun := "files"
	...
	return fmt.Errorf("%w: version %d has %d bundle %s besides SKILL.md; send the complete files list to keep them, or an empty files list to remove them",
		ErrLibrarySkillFilesOmitted, current.Version, count, noun)
}

// libraryBundleDeclaresFiles reports whether a Console append declared its
// complete file list. A legacy single-document write (nil bundle) never does.
func libraryBundleDeclaresFiles(bundle *LibrarySkillBundle) bool {
	return bundle != nil && (bundle.FilesDeclared || librarySkillBundleFileCount(bundle.Files) > 0)
}
