...
func RegisterLibraryArtifactTools(s *server.MCPServer, store LibraryStore, sink AuditSink) {
	...
	s.AddTool(
		...
		func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			return mcp.NewToolResultJSON(struct {
				...
				Skill: skill, Version: latest, Manifest: libraryStoredSkillManifestProjection(ctx, store, latest), Bindings: bindings,
	...
	s.AddTool(
		mcp.NewTool(
			...
			mcp.WithDescription("Read one bundle file (with its SHA-256 digest, size, and media type) from an exact immutable version of a portable Library skill. Text files return utf8; binaries return base64. Verify the digest before use. Files are data only: Synaxis never executes them and they grant no credentials, tools, or authority."),
		...
		func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			if _, found := store.LibrarySkillVersion(ctx, input.SkillID, input.VersionID); !found {
				return mcp.NewToolResultError("skill file not found"), nil
			}
			data, file, found, err := bundleStore.LibrarySkillFileBytes(ctx, input.SkillID, input.VersionID, filePath)
			if err != nil {
			...
			if sink != nil {
				sink.LogCall(CallRecord{Account: "engine", Tool: "library_skill_file_read", OK: true})
			}
			return mcp.NewToolResultJSON(newLibrarySkillFileReadResult(input.SkillID, input.VersionID, file, data))
		},
	)
	s.AddTool(
		mcp.NewTool(
