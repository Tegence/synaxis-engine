...
func RegisterLibraryArtifactTools(s *server.MCPServer, store LibraryStore, sink AuditSink) {
	...
	s.AddTool(
		mcp.NewTool(
			...
			mcp.WithDescription("Resolve portable Library skill instructions for a trusted opaque runtime context. The result is read-only and does not grant credentials, permissions, OAuth scopes, tools, or connection access."),
...
type librarySkillResolveInput struct {
	BindingID    string `json:"bindingId,omitempty" jsonschema:"Optional explicit Library binding ID; overrides scope fields"`
	FolderID     string `json:"folderId,omitempty" jsonschema:"Opaque folder context ID"`
	RepositoryID string `json:"repositoryId,omitempty" jsonschema:"Opaque repository context ID"`
	NamespaceID  string `json:"namespaceId,omitempty" jsonschema:"Opaque generic namespace context ID; not a credential namespace"`
	WorkspaceID  string `json:"workspaceId,omitempty" jsonschema:"Opaque workspace context ID"`
}
