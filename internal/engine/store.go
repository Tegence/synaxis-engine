...
type FileStore struct {
	...
	// librarySkillBlobs is the content-addressed store for bundle file bytes
	// (every file except SKILL.md, whose bytes stay on the version). Blobs are
	// keyed by SHA-256 and shared by every version that references them.
	librarySkillBlobs []*librarySkillBlobRecord
	// librarySkillBlobStagings authorizes a leased MCP client to reference a
	// blob it uploaded; the record expires with the lease and is never a
	// bearer capability for the blob bytes.
	librarySkillBlobStagings []*librarySkillBlobStagingRecord
	...
	// Artifact drafts and their import receipts mirror the skill draft pair:
	// editable generated output candidates plus hash-only replay records.
	libraryArtifactDrafts       []*LibraryArtifactDraft
	libraryArtifactDraftImports []*librarySkillDraftImportRecord
...
type fileStoreData struct {
	...
	LibrarySkillBlobs                         []*librarySkillBlobRecord                      `json:"library_skill_blobs,omitempty"`
	LibrarySkillBlobStagings                  []*librarySkillBlobStagingRecord               `json:"library_skill_blob_stagings,omitempty"`
	...
	LibraryArtifactDrafts                     []*LibraryArtifactDraft                        `json:"library_artifact_drafts,omitempty"`
	LibraryArtifactDraftImports               []*librarySkillDraftImportRecord               `json:"library_artifact_draft_imports,omitempty"`
...
func LoadFileStore(path string) (*FileStore, error) {
	...
	s.librarySkillBlobs, s.librarySkillBlobStagings = d.LibrarySkillBlobs, d.LibrarySkillBlobStagings
	if err := s.materializeLibrarySkillManifestsLocked(); err != nil {
		return nil, fmt.Errorf("migrate library skill manifests: %w", err)
	}
	s.librarySkillDraftImports = d.LibrarySkillDraftImports
	s.libraryArtifactDrafts, s.libraryArtifactDraftImports = d.LibraryArtifactDrafts, d.LibraryArtifactDraftImports
...
func (s *FileStore) saveLocked() error {
	b, err := json.MarshalIndent(fileStoreData{
		...
		LibrarySkillBlobs:                         s.librarySkillBlobs,
		LibrarySkillBlobStagings:                  s.librarySkillBlobStagings,
		LibrarySkillDrafts:                        s.librarySkillDrafts,
		LibrarySkillDraftImports:                  s.librarySkillDraftImports,
		LibraryArtifactDrafts:                     s.libraryArtifactDrafts,
		LibraryArtifactDraftImports:               s.libraryArtifactDraftImports,
