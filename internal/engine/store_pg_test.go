...
// TestPgStoreRecentCallsBeforePagesCompleteAndOrdered proves the keyset SQL
// predicate — WHERE (ts,id) < ($2,$3) ORDER BY ts DESC, id DESC — pages
// backward through a fixture completely, in order, and that two rows sharing
// one exact timestamp break the tie on id. The exhaustive 250-row sweep lives
// in the ring test (audit_test.go); this proves the same contract holds
// against real Postgres SQL. The traversal starts from an explicit cursor
// strictly newer than the whole fixture (rather than RecentCalls) and filters
// by account so the test is immune to whatever else a shared integration
// database holds.
func TestPgStoreRecentCallsBeforePagesCompleteAndOrdered(t *testing.T) {
	dsn := os.Getenv("TEST_DATABASE_URL")
	if dsn == "" {
		t.Skip("set TEST_DATABASE_URL to run the Postgres integration test")
	}
	...
	defer s.Close()
	defer s.pool.Exec(ctx, `DELETE FROM tool_calls WHERE account='fr-page-test'`)

	base := time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC)
	const rowCount = 12
	for i := 0; i < rowCount; i++ {
		if _, err := s.pool.Exec(ctx,
			`INSERT INTO tool_calls (ts,account,tool,ok,ms) VALUES ($1,'fr-page-test','t',true,1)`,
			base.Add(time.Duration(i)*time.Second),
		); err != nil {
			t.Fatalf("seed row %d: %v", i, err)
		}
	}
	// Two rows sharing one exact timestamp, strictly after the rest — id must
	// break the tie deterministically.
	tieTS := base.Add(time.Duration(rowCount) * time.Second)
	for i := 0; i < 2; i++ {
		if _, err := s.pool.Exec(ctx,
			`INSERT INTO tool_calls (ts,account,tool,ok,ms) VALUES ($1,'fr-page-test','tie',true,1)`,
			tieTS,
		); err != nil {
	...
	filterMine := func(in []CallRecord) []CallRecord {
		out := make([]CallRecord, 0, len(in))
		for _, c := range in {
			if c.Account == "fr-page-test" {
				out = append(out, c)
			}
		}
		return out
	...
	const pageSize = 5
	cursorTS, cursorID := tieTS.Add(time.Second), int64(math.MaxInt64)
	var all []CallRecord
	for i := 0; i < 100; i++ { // generous cap; the predicate strictly shrinks the cursor each hop
		page, err := s.RecentCallsBefore(ctx, cursorTS, cursorID, pageSize)
		if err != nil {
			t.Fatalf("RecentCallsBefore: %v", err)
		}
		all = append(all, filterMine(page)...)
	...
	if len(all) != total {
		t.Fatalf("traversal visited %d fr-page-test rows, want %d", len(all), total)
	}
	...
	for i, c := range all {
		...
		seen[c.ID] = true
		if i > 0 && !keysetBefore(c.TS, c.ID, all[i-1].TS, all[i-1].ID) {
			t.Fatalf("row %d out of order: prev=%+v cur=%+v", i, all[i-1], c)
		}
	}
	// The two tied rows must both be present, newest (highest id) first.
	if all[0].Tool != "tie" || all[1].Tool != "tie" || all[0].ID <= all[1].ID {
		t.Fatalf("same-timestamp tie not broken by id, newest first: %+v, %+v", all[0], all[1])
	}
}

// TestPgStoreRecentCallsThenRecentCallsBeforeTraversalIsGapFreeAndDupeFree is
// the regression test for the page-1/page-2 boundary bug: RecentCalls (used
// for the FIRST /api/logs page) had no id tie-break, while RecentCallsBefore
// (used for every "Load older" page after it) orders ts DESC, id DESC. Rows
// sharing an exact timestamp that straddle that boundary could therefore
// come back in a different relative order from each query — duplicating some
// ids across the two pages and permanently losing others. This seeds five
// rows on one exact timestamp positioned right at that boundary, pages
// through via RecentCalls then RecentCallsBefore exactly as the console
// handler does, and asserts the traversal is complete, gap-free, and
// duplicate-free.
func TestPgStoreRecentCallsThenRecentCallsBeforeTraversalIsGapFreeAndDupeFree(t *testing.T) {
	dsn := os.Getenv("TEST_DATABASE_URL")
	if dsn == "" {
		t.Skip("set TEST_DATABASE_URL to run the Postgres integration test")
	}
	...
	defer s.Close()
	defer s.pool.Exec(ctx, `DELETE FROM tool_calls WHERE account='fr-boundary-test'`)

	// A handful of older, distinctly-timestamped rows, then five rows sharing
	// one exact timestamp at the page-1/page-2 boundary, then a couple more
	// distinctly-timestamped rows newer still so the tied rows are neither
	// the very newest nor the very oldest in the fixture.
	base := time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC)
	const olderCount = 3
	for i := 0; i < olderCount; i++ {
		if _, err := s.pool.Exec(ctx,
			`INSERT INTO tool_calls (ts,account,tool,ok,ms) VALUES ($1,'fr-boundary-test','older',true,1)`,
			base.Add(time.Duration(i)*time.Second),
		); err != nil {
			t.Fatalf("seed older row %d: %v", i, err)
		}
	}
	tieTS := base.Add(time.Duration(olderCount) * time.Second)
	const tieCount = 5
	for i := 0; i < tieCount; i++ {
		if _, err := s.pool.Exec(ctx,
			`INSERT INTO tool_calls (ts,account,tool,ok,ms) VALUES ($1,'fr-boundary-test','tie',true,1)`,
			tieTS,
		); err != nil {
			t.Fatalf("seed tie row %d: %v", i, err)
		}
	...
	for i := 0; i < newerCount; i++ {
		if _, err := s.pool.Exec(ctx,
			`INSERT INTO tool_calls (ts,account,tool,ok,ms) VALUES ($1,'fr-boundary-test','newer',true,1)`,
			tieTS.Add(time.Duration(i+1)*time.Second),
		); err != nil {
			t.Fatalf("seed newer row %d: %v", i, err)
		}
	}
	total := olderCount + tieCount + newerCount

	filterMine := func(in []CallRecord) []CallRecord {
		out := make([]CallRecord, 0, len(in))
		for _, c := range in {
			if c.Account == "fr-boundary-test" {
				out = append(out, c)
			}
		}
		return out
	...
	// Page size chosen so the tied rows straddle the RecentCalls/
	// RecentCallsBefore boundary: RecentCalls(pageSize) returns the newest
	// pageSize rows (newerCount=2 plus 3 of the 5 tied rows); the "Load
	// older" cursor then starts mid-tie.
	const pageSize = 5
	firstPage, err := s.RecentCalls(ctx, pageSize)
	if err != nil {
		t.Fatalf("RecentCalls: %v", err)
	}
	var all []CallRecord
	all = append(all, filterMine(firstPage)...)
	page := firstPage
	for len(page) >= pageSize {
		last := page[len(page)-1]
		page, err = s.RecentCallsBefore(ctx, last.TS, last.ID, pageSize)
		if err != nil {
			t.Fatalf("RecentCallsBefore: %v", err)
		}
		all = append(all, filterMine(page)...)
	...
	if len(all) != total {
		t.Fatalf("traversal visited %d fr-boundary-test rows, want %d (duplicated or lost rows across the page-1/page-2 boundary)", len(all), total)
	}
	seen := make(map[int64]bool, total)
	for i, c := range all {
		if seen[c.ID] {
			t.Fatalf("row id=%d visited more than once (duplicated across pages): %+v", c.ID, all)
		}
		seen[c.ID] = true
		if i > 0 && !keysetBefore(c.TS, c.ID, all[i-1].TS, all[i-1].ID) {
			t.Fatalf("row %d out of order: prev=%+v cur=%+v", i, all[i-1], c)
	...
	if tieSeen != tieCount {
		t.Fatalf("saw %d of the %d same-timestamp boundary rows across the full traversal, want all %d", tieSeen, tieCount, tieCount)
	}
}
