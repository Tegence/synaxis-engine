...
func VerifyActivationBundle(activation ActivationBundle, limits Limits) (VerifiedActivationBundle, error) {
	...
	if activation.ContractVersion != ActivationContractVersion {
	...
	if activation.AuthorityNotice != activationAuthorityNotice || !sameStrings(activation.HostResponsibilities, activationHostResponsibilities) {
	...
	for index, skill := range activation.Skills {
		if err := validateSkill(skill, resolvedLimits); err != nil {
			return VerifiedActivationBundle{}, err
		}
...
func BuildContext(activation VerifiedActivationBundle, limits Limits) (Context, error) {
	...
	if activation.bundle.ContractVersion != ActivationContractVersion {
